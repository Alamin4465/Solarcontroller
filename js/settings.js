// js/settings.js - ESP32 v6.7.2 aligned defaults
// Battery SOC: Linear (11.0V = 0%, 13.7V = 100%)
// Battery Full: 13.7V | Recover: 13.0V | Min: 11.8V
// Solar: Good 13.5V | Min 12.5V

// ==================== ডিফল্ট ক্লিনিং সেটিংস ====================
const DEFAULT_CLEANING_SETTINGS = {
    duration: 30,        // সেকেন্ড (ESP32 default: 5s but Web App 30s)
    interval: 6,         // ঘণ্টা
    cycles: 3,           // সংখ্যা
    breakTime: 10        // সেকেন্ড
};

// ✅ ডিফল্ট Battery Cutoff (ESP32 aligned)
const DEFAULT_BATTERY_CUTOFF = {
    fullVoltage: 13.7,
    recoverVoltage: 13.0
};

// ✅ ডিফল্ট Thresholds (ESP32 v6.7.2 aligned)
const DEFAULT_THRESHOLDS = {
    SOLAR_MIN_VOLTAGE: 12.5,        // ESP32: SOLAR_MIN_V
    SOLAR_GOOD_VOLTAGE: 13.5,       // ✅ was 13.0 → ESP32: SOLAR_GOOD_V = 13.5
    BATTERY_MIN_VOLTAGE: 11.8,      // ✅ was 11.5 → ESP32: BATTERY_MIN_V = 11.8
...
// ✅ AUTO_THRESHOLDS global (ESP32 aligned)
            ...
            if (snapshot.exists()) {
                ...
                if (settings.auto_mode_thresholds) {
                    ...
                    if (window.AUTO_THRESHOLDS) {
                        ...
                        window.AUTO_THRESHOLDS.SOLAR_GOOD_VOLTAGE = thresholds.SOLAR_GOOD_VOLTAGE || 13.5;   // ✅ 13.5
                        window.AUTO_THRESHOLDS.BATTERY_MIN_VOLTAGE = thresholds.BATTERY_MIN_VOLTAGE || 11.8; // ✅ 11.8
                ...
                // ✅ Battery Cutoff Settings লোড
                if (settings.battery_cutoff) {
                    window.batteryCutoffSettings = {
                        fullVoltage: settings.battery_cutoff.full_voltage || 13.7,
                        recoverVoltage: settings.battery_cutoff.recover_voltage || 13.0
                    };
...
function saveDefaultSettingsToFirebase() {
    ...
    const defaultSettings = {
        auto_mode_thresholds: {
            SOLAR_MIN_VOLTAGE: 12.5,
            SOLAR_GOOD_VOLTAGE: 13.5,           // ✅ 13.5
            BATTERY_MIN_VOLTAGE: 11.8,          // ✅ 11.8
            BATTERY_CRITICAL_SOC: 25,
            BATTERY_GOOD_SOC: 40,
...
function getCurrentThresholds() {
    if (window.AUTO_THRESHOLDS) {
        return {
            ...
            SOLAR_GOOD_VOLTAGE: window.AUTO_THRESHOLDS.SOLAR_GOOD_VOLTAGE || 13.5,
...
function saveThresholdsToFirebase() {
    ...
    const solarGood = parseFloat(document.getElementById('solarGoodVoltage')?.value || 13.5);
    ...
    window.update(window.ref(database, 
        ...
        .then(() => {
            ...
            window.showNotification('থ্রেশহোল্ড সংরক্ষণ ✅ — ESP32 এ পাঠানো হয়েছে', 'success');
        })
        .catch((error) => {
...
function saveCleaningSettingsToFirebase() {
    ...
    window.update(window.ref(database, 
        ...
        .then(() => {
            ...
            window.showNotification('ক্লিনিং সেটিংস সংরক্ষণ ✅ — ESP32 এ পাঠানো', 'success');
        })
        .catch((error) => {
            console.error("Error:", error);
            window.showNotification('সংরক্ষণে সমস্যা ❌', 'error');
...
function saveBatteryCutoffToFirebase() {
    const fullV = parseFloat(document.getElementById('batteryFullVoltage')?.value || 13.7);
    const recoverV = parseFloat(document.getElementById('batteryRecoverVoltage')?.value || 13.0);
    ...
    window.update(window.ref(database, 
        ...
        .then(() => {
            ...
            window.showNotification('ব্যাটারি কাটঅফ সংরক্ষণ ✅ — ESP32 এ পাঠানো', 'success');
        })
        .catch((error) => {
            console.error("Error:", error);
            window.showNotification('সংরক্ষণে সমস্যা ❌', 'error');
...
function changeWiFi() {
    ...
    if (window.pushCommand) {
        window.pushCommand('wifi_config', {
            ...
            window.showNotification('ওয়াইফাই কমান্ড পাঠানো হয়েছে (ESP32 v6.7.2+: ignore)', 'info');
...
function restartDevice() {
    ...
    if (window.pushCommand) {
        window.pushCommand('system_config', { command: 'restart' })
            .then(() => window.showNotification('রিস্টার্ট কমান্ড পাঠানো (v6.7.2+: ignore)', 'info'))
            .catch(() => window.showNotification('কমান্ড পাঠাতে সমস্যা', 'error'));
    }
}
...
function esp32FactoryReset() {
    ...
    if (window.pushCommand) {
        ...
        }).then(() => window.showNotification('ESP32 ফ্যাক্টরি রিসেট কমান্ড পাঠানো (v6.7.2+: ignore)', 'warning'))
          .catch(() => window.showNotification('কমান্ড পাঠাতে সমস্যা', 'error'));
    }
}
    ...
    document.getElementById('clearHistoryBtn')?.addEventListener('click', () => {
        if (confirm(`⚠️ ${selectedOption === 'all' ? 'সব ডাটা' : selectedOption + ' দিনের পুরনো ডাটা'} ডিলিট করবেন?`)) {
            if (window.pushCommand) {
                ...
                }).then(() => window.showNotification('হিস্ট্রি ক্লিয়ার কমান্ড পাঠানো (v6.7.2+: manual Firebase delete)', 'warning'))
                  .catch(() => window.showNotification('কমান্ড পাঠাতে সমস্যা', 'error'));
            }
        }
    });
...
console.log('✅ Settings.js v2.0 - ESP32 v6.7.2 aligned');